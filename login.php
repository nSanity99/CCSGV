...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['login'])) {
    ...
    if ($conn->connect_error) {
        ...
        // Recupera anche il gruppo di lavoro dell'utente per determinare quali app mostrare
        $stmt = $conn->prepare("SELECT id, username, password_hash, ruolo, nome, gruppo_lavoro FROM utenti WHERE username = ?");
        if ($stmt) {
            ...
            if (!$stmt->execute()) {
                ...
                $stmt->store_result();
                if ($stmt->num_rows > 0) {
                    // Oltre alle informazioni principali otteniamo anche il gruppo di lavoro
                    $stmt->bind_result($user_id, $db_username, $hashed_password_from_db, $db_ruolo, $db_user_fullname, $db_gruppo_lavoro);
                    $stmt->fetch();
...
/* Footer Crediti */
.footer-credits {
    position: absolute;           /* fissa il footer in fondo */
    bottom: 0;
    left: 0;
    width: 100%;
    padding: 15px 10px;
...
<body>
    ...
    <p><span class="emoji"></span> Realizzato  dall’Ufficio IT — Contact Centre Sud  In Collaborazione con Robin Santoro </p>
    <p><span class="emoji">©</span> 2025 Gruppo Vitolo — Tutti i diritti riservati</p>
</footer>
